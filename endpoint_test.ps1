$u = (Invoke-RestMethod http://localhost:8080/spotify/login).spotify_auth_uri
$u | Set-Clipboard
...
(Invoke-RestMethod http://localhost:8080/spotify/authenticated) # skaffa access token kräver ?code=

$a = (Invoke-RestMethod http://localhost:8080/spotify/login/refresh?refresh_token=$r) # förnya access token kräver ?refresh_token=
